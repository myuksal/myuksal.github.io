...
이더리움은 **Beacon 체인** 기반의 **지분증명(Proof of Stake, PoS)** 합의 알고리즘을 채택하고 있습니다. 합의 프로토콜은 \*\*에포크(epoch)\*\*와 **슬롯(slot)** 개념으로 운영되며, **1개 에포크는 32개 슬롯** (약 6.4분)으로 구성되고 **1개 슬롯은 12초**로 정의됩니다. 각 슬롯마다 네트워크의 검증인(validator) 중 한 명이 **블록 제안자**로 무작위 선택되어 블록을 제안하고, 나머지 검증인들은 해당 슬롯의 블록에 대한 \*\*승인 투표(attestation)\*\*를 수행합니다. 블록 제안이 없는 빈 슬롯도 발생할 수 있으며, 이 경우 체인은 다음 슬롯으로 넘어갑니다.

PoS 합의의 **포크 선택 규칙**은 **LMD-GHOST**(Latest Message Driven - Greediest Heaviest Observed SubTree) 알고리즘과 **Casper FFG** 최종화(finality) 알고리즘을 결합한 형태로 동작합니다. 즉, **검증인들의 최신 투표 메시지에 기반하여 가장 무거운 서브트리를 선택**하는 LMD-GHOST를 사용하되, **마지막으로 최종화된 체크포인트 블록의 자손들만 유효한 후보로 간주**하도록 Casper FFG 규칙이 LMD-GHOST를 제약합니다. Casper FFG의 투표를 통해 에포크 단위로 \*\*블록의 정당성(Justification)과 최종성(Finalization)\*\*이 확보되며, 한 번 \*\*최종화(finalized)\*\*된 블록 이전의 체인 분기는 영구적으로 포크 선택 대상에서 제외됩니다. 결국 **최종화된 체크포인트 이후 구간에서는 LMD-GHOST에 따라 현재 가장 높은 투표 가중치를 지닌 체인 헤드가 선택**됩니다. 이러한 구조 덕분에 PoS 이더리움은 \*\*체인 재구성 깊이(reorg depth)\*\*가 제한되며 (최종화된 블록 이전으로는 재구성 불가), **안전성**과 **활용성**을 모두 확보합니다.

또한 \*\*랜덤 비콘(randomness beacon)\*\*으로서 **RANDAO** 메커니즘이 합의에 사용됩니다. 각 슬롯의 블록 제안자는 자신의 개인키로 현재 에포크 번호에 서명한 값을 \*\*Randao 리빌(reveal)\*\*로 블록에 포함하며, 이를 Beacon 체인이 누적하여 난수 시드(seed)를 생성합니다. Beacon 체인 명세에 따르면 **블록 제안자의 Randao 서명이 올바르지 않으면 블록은 무효**로 간주됩니다. 이렇게 수집된 난수는 다음 에포크의 검증인 셔플링 등에 활용됩니다. PoS 이더리움 Merge 이후 실행층(EVM)에서도 이 Randao 값을 활용할 수 있도록 합의가 변경되었는데, 이에 대해서는 각 클라이언트의 **RANDAO 구현 로직**에서 자세히 설명합니다.

요약하면, **이더리움 PoS 합의 알고리즘**은 Beacon 체인의 **슬롯/에포크 스케줄**에 따라 **검증인들이 블록 제안 및 투표**를 수행하고, **Casper FFG**로 **최종성**을 부여하며, **LMD-GHOST**로 **비최종화 구간의 최상위 체인 헤드**를 선택하는 구조입니다. 다음 섹션들에서는 이러한 프로토콜을 실행 클라이언트인 **Geth**와 **Reth**에서 어떻게 구현하고 있는지 살펴보겠습니다.

## 2. Geth 클라이언트에서의 PoS 합의 구현

### 2.1 포크 선택 규칙 처리 (Fork Choice Rule in Geth)

Geth는 \*\*실행 클라이언트(Execution Client)\*\*로서, PoS 합의 프로토콜의 **포크 선택 결과**를 **엔진 API**를 통해 **컨센서스 클라이언트**로부터 전달받아 처리합니다. Merge 이후 Geth에는 `engine_forkchoiceUpdated` 등의 엔진 API RPC 핸들러가 추가되어, Beacon 체인 합의層이 새로운 체인 헤드를 선택할 때 이를 받아들여 **체인 상태를 전환**합니다. Geth 소스코드에서는 `eth/catalyst/api.go` 모듈의 `ConsensusAPI` 구조체에 이러한 엔진 API 핸들러들이 구현되어 있습니다. 예를 들어 `ConsensusAPI.ForkchoiceUpdatedV1(...)` 메서드는 컨센서스 레이어로부터 **포크 선택 상태와 신규 페이로드 지시를 입력받아**, 내부적으로 `api.forkchoiceUpdated(...)` 함수를 호출함으로써 Geth의 체인 헤드를 갱신하고 (필요시) 블록 생성 프로세스를 시작합니다.

Geth는 **포크 선택(Fork Choice)** 처리를 위해 **ForkChoice 구조체**를 도입하여 체인 헤드 전환을 관리합니다. `core.BlockChain` 구조체는 `forker` 필드로 `ForkChoice` 인스턴스를 유지하며, Geth 실행 시 `NewForkChoice(bc, preserveFunc)`를 호출하여 이를 초기화합니다. 이 `ForkChoice`는 현재 체인의 정보(`ChainReader` 인터페이스)와 블록 보존 조건 함수(`preserve` 함수)를 받아 구성되며, 핵심 메서드로 \*\*`ReorgNeeded()`\*\*를 제공합니다.

- `ForkChoice.ReorgNeeded(current *types.Header, extern *types.Header) (bool, error)`는 **현재 캐노니컬 헤드**(current)와 **외부에서 제시된 새로운 헤드**(extern)를 비교하여 \*\*체인 재구성(reorg)\*\*이 필요한지 여부를 판단합니다. 이 함수는 extern 헤더가 현재 체인의 연장선에 있는 경우 단순 연장으로 보고 reorg 불필요하다고 판단하며, extern이 다른 분기를 나타낼 경우 `true`를 반환하여 재구성 필요성을 알립니다. 또한 extern 헤더가 DB에 없는 등 **조상이 불명확한 경우 에러**를 리턴하여, Geth가 컨센서스 레이어에 **동기화 필요**(SYNCING 상태) 응답을 할 수 있도록 합니다. Geth의 forkchoice 처리 로직은 이러한 `ReorgNeeded` 결과를 토대로, 필요시 체인 재구성을 수행하고 새로운 헤더를 캐노니컬 헤드로 **승격**시킵니다.

Geth 엔진 API 핸들러는 **컨센서스 클라이언트가 전달한 포크 선택 상태**를 수신하면, 우선 해당 **헤드 블록 해시**가 가리키는 블록을 자체 DB에서 조회합니다. 블록이 이미 삽입되어 있다면(예: 이전에 `engine_newPayload`를 통해 검증/저장 완료), 곧바로 `ForkChoice`를 활용해 현재 헤드와 비교한 뒤 **체인 헤드 포인터를 갱신**합니다. 반면 블록 정보가 없다면 컨센서스 쪽에 **동기화 필요 상태**를 보고하고, 필요한 페이로드들을 받아와 블록을 저장한 후 헤드를 갱신합니다. Geth는 forkchoice 업데이트 처리 과정에서 \*\*안전 헤드(safe block)\*\*와 **최종화된 블록(finalized block)** 정보도 함께 제공받는데, 이에 대해서는 뒷부분(에포크/슬롯 관리)에서 설명합니다. 요약하면, **Geth는 PoS의 포크 선택 알고리즘 자체(LMD-GHOST)는 구현하지 않지만**, **Engine API를 통해 전달된 최종 포크 선택 결과를 반영**하여 **현재 체인 헤드**를 전환하고, 필요 시 **새 블록을 빌드**하거나 **동기화**하는 역할을 합니다.

### 2.2 체인 재구성 (Reorg) 처리 in Geth

체인 \*\*재구성(reorganization)\*\*이란 **현재 선택된 캐논ical 체인을 다른 분기로 변경**하는 과정으로, PoS 체인에서는 주로 **포크 선택 규칙**에 의해 새로운 헤드가 기존 헤드의 **조상보다 과거의 블록에서 갈라진 분기일 때** 발생합니다. Geth에서는 앞서 언급한 `ForkChoice` 모듈과 기존의 체인관리 코드를 활용하여 reorg를 처리합니다.

Geth의 `core.BlockChain` 구조체는 블록을 DB에 저장하고 체인 상태를 관리하는데, 여기에는 `SetHead`/`SetFinalized` 등의 함수와 내부적으로 **reorg 시 분기 정리** 로직이 구현되어 있습니다. 예를 들어 \*\*`BlockChain.SetFinalized(header)`\*\*는 주어진 헤더를 **최종화 블록**으로 설정하면서, 해당 블록보다 이전의 모든 분기 데이터를 정리하도록 설계되어 있습니다. 마찬가지로 \*\*`BlockChain.SetSafe(header)`\*\*는 \*\*안전 블록(safe head)\*\*를 설정하여 체인 재구성 한계를 표시합니다. 이러한 함수들은 Geth의 Engine API 처리 루틴에서, 컨센서스 클라이언트가 전달한 finalized 블록 및 safe 블록 해시를 실제 헤더로 찾아 설정하는데 사용됩니다.

실제 reorg 발생 시 Geth는 \*\*공통 조상 블록(common ancestor)\*\*을 찾은 뒤, 기존 체인의 해당 조상 이후 블록들을 \*\*canonical 체인에서 제거(언클리화)\*\*하고 신규 체인의 블록들을 **canonical 체인으로 편입**합니다. Geth는 이 과정을 수행할 때 **ChainHeadEvent**와 **ChainSideEvent** 등을 발생시켜 상위 모듈에 체인 전환을 알리고, 트랜잭션 풀 등 부가 구성요소를 업데이트합니다. `ForkChoice.ReorgNeeded(...)`가 `true`를 반환하면, Geth는 일반적으로 새로운 헤더를 현재 헤드로 만들기 위해 `BlockChain` 내부에서 재구성 루틴을 호출하며, 이 과정에서 **상태 트라이(state trie)** 롤백/적용과 **메모리 캐시 갱신** 등이 수반됩니다.

특히 PoS 상황에서는 **최종화된 블록** 이전으로의 reorg가 불가능하도록 보장되므로, Geth는 reorg 시 항상 **최종화 지점 이후**의 블록들만 대체합니다. Geth는 `currentFinalBlock`과 `currentSafeBlock` 포인터를 관리하면서, reorg하려는 새로운 헤드가 이들 제약을 어기는지 검사합니다. 만약 컨센서스 레이어로부터 전달된 새로운 헤드가 현재 최종화 블록 이전의 분기를 가리킨다면, Geth는 이를 거부하거나 오류로 처리하게 됩니다 (정상적인 컨센서스 구현에서는 이러한 상황이 발생하지 않음).

**정리하면**, Geth의 reorg 처리는 다음과 같은 흐름으로 요약됩니다:
...
이 모든 과정은 Geth `core.BlockChain` 모듈 내에 구현되어 있으며, PoW 시절의 체인관리 코드를 기반으로 하되 PoS의 **최종성** 개념을 반영하여 업데이트되었습니다. Geth 1.11+ 버전의 구현을 보면, `BlockChain` 초기화 시 `forker`(ForkChoice) 설정, `currentFinalBlock`/`currentSafeBlock` 초기화 등이 이뤄지고, `loadLastState()` 호출로 재시작 시 디스크에 저장된 마지막 헤드/최종화 지점을 불러옵니다. 이러한 구조를 통해 Geth는 재시작해도 이전에 받은 최종화 블록 정보를 기억하고, 잘못된 깊이의 reorg 시도를 방지합니다.
